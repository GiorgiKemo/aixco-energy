  ...
  return (
    <main className="pt-24 lg:pt-32 min-h-screen bg-industrial-white text-industrial-black">
      <section className="px-6 pb-24 pt-16 md:py-28 max-w-7xl mx-auto">
        <div className="mb-24">
          <div className="mb-6 text-sm font-black uppercase tracking-[0.18em] text-brand-red md:text-base">{tx(whyCopy.label)}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-800 border border-zinc-800 mb-24">
          {platformMetrics.map((metric) => (
            <div key={metric.label} className="bg-zinc-950 p-8">
              ...
              <p className="text-sm font-black uppercase leading-relaxed text-zinc-500">{tx(metric.label)}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          <div className="min-w-0 lg:col-span-5">
            <div className="mb-6 text-sm font-black uppercase tracking-[0.18em] text-brand-red md:text-base">{tx(strategyCopy.label)}</div>
            <h2 className="mb-8 break-words text-[clamp(2.5rem,5vw,5rem)]">{tx(strategyCopy.title)}</h2>
          </div>
          <div className="min-w-0 lg:col-span-7">
            <p className="text-zinc-500 text-sm font-black uppercase leading-relaxed mb-10">{tx(strategyCopy.body)}</p>
