    ...
    >
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 lg:grid-cols-12 lg:items-start">
        <div className="min-w-0 lg:col-span-5">
          <div className="mb-5 text-sm font-black tracking-[0.18em] text-brand-red md:text-base">{tx(investorReasons.label)}</div>
          ...
          <div className="mt-10 flex items-center gap-3 text-brand-red">
            <Network size={20} />
            <span className="text-sm font-black tracking-normal">{tx("Investor FAQs")}</span>
          </div>
        </div>
        ...
        <div className="min-w-0 lg:col-span-7">
          <ul className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {investorReasons.bullets.map((bullet) => (
              <li key={bullet} className="flex gap-3 border border-zinc-800 bg-zinc-950 p-5 text-sm font-black leading-relaxed text-zinc-500">
